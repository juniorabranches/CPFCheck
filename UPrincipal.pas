...
procedure TForm1.ChromiumCPFConsoleMessage(Sender: TObject;
  ...
  out Result: Boolean);
begin
   if pos('Nº do CPF', message) > 0 then
   ...
   if pos('Situação Cadastral', message) > 0 then
      EditSituacao.Text := copy(message,21,length(message));

   if pos('Dígito Verificador', message) > 0 then
      EdtDigitoVerificador.Text := copy(message,21,length(message));

   if  pos('Código de controle do comprovante', message) > 0 then
      EdtCodCtrlControle.Text := copy(message,36,length(message));

   if  pos('Comprovante emitido às', message) > 0 then
      EdtEmissao.Text := copy(message,25,length(message));
end;

procedure TForm1.FormShow(Sender: TObject);
begin
   Form1.Caption := 'Sincronizar Cadastro com Receita Federal - Pessoa Física';
   NovaConsultaCPF;
end;
